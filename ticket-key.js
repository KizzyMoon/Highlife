...
function mount(){
  ...
  if(!home||document.getElementById('ticket-routing-key')) return;
  const target=home.querySelector('.dashboard-grid');
  if(!target)return;
  ...
  if(backup&&backup.parentElement===target)target.insertBefore(card,backup);else target.appendChild(card);
}
